...
const TabTrashed = () => {
  ...
  useEffect(() => {
    fetch('https://apisvi.dofazcode.id/api/getArticle?status=published').then((response) => response.json()).then((data) => {console.log(data.newArticle)
  ...
  const handleTrash = (id) => {
    ...
    if (confirmDelete) {
      fetch(`https://apisvi.dofazcode.id/api/deleteArticle/${id}`, {
        method: 'DELETE',
      })
      .then((response) => {
        if (!response.ok) {
